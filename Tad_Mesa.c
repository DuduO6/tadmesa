...
void CarregarBaralhoAleatorio(Mesa* mesa) {
    // Array de cartas que representam o baralho completo
    Carta baralhoCompleto[52];

    // Preencha o array com todas as cartas possíveis
    int index = 0;
    for (int naipe = 0; naipe < 4; naipe++) {
        for (int valor = 0; valor < 13; valor++) {
            baralhoCompleto[index] = criarCarta(valor, naipe, 0);
            index++;
        }
    }
    ...
    for (int i = 51; i > 0; i--) {
        ...
        // Troque as cartas i e j
        Carta temp = baralhoCompleto[i];
        baralhoCompleto[i] = baralhoCompleto[j];
        baralhoCompleto[j] = temp;
    }

    // Adicione as cartas embaralhadas à lista de cartas do baralho da mesa
    for (int i = 0; i < 52; i++) {
        AdicionarNoTopo(&mesa->baralho, baralhoCompleto[i]);
    }
}
...
void MoverBasesParaTableau(Mesa* mesa, int indiceBase, int indiceTableau) {
    ...
    if (Tamanho(&(mesa->tableau[indiceTableau].cartas)) == 0 || verificarSequenciaAlternada(cartaBase, &(mesa->tableau[indiceTableau].cartas->topo->carta))) {
        ...
        AdicionarNoTopo(&(mesa->tableau[indiceTableau].cartas), cartaBase);
        printf("A carta foi movida para o tableau %d.\n", indiceTableau + 1);
    } else {
